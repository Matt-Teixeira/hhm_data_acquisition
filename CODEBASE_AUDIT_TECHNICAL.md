...
### SEC-011 — Historical plaintext device credentials persist in three DB sinks, world-readable disk archives, and an unauthenticated dashboard view
- **Found:** 2026-09-08, during the four-day verification of SEC-002. The redactor has been **100% effective since it shipped** (every post-`d13b9d4` row masked: 27,750 checked, 0 unmasked), but everything written *before* it is still there.
- **DB sinks (measured dry run, 2026-09-08):** `util.app_run_logs` partitions `2026_08`/`2026_09` — 3,760 rows, 55,471 credential occurrences in each of `verbose_log` and `warn_error_logs`; `incidents.error_events` — 55,471 rows in both `err_msg` and `raw_event` (the incident engine copied every event); `incidents.incidents.sample_message` — 317 incidents. The three counts agree exactly.
- **Disk sink:** the 15 daily archives `2026-08-19..2026-09-02.tar.gz` under `/opt/run-logs/data_acquisition/archive/` hold **36,109** occurrences, are **world-readable** (`-rw-rw-r--`), and are kept **180 days** by `prune-run-logs.sh`. Loose per-run logs since 09-07: 0 unmasked (5,799 masked).
- **Screen:** ops-dashboard maps the incident event `message` from `err_msg` ([lib/incidents.js:185](../ops-dashboard/lib/incidents.js#L185)) and renders it as expandable text ([public/index.html:2640](../ops-dashboard/public/index.html#L2640)); the app has **no authentication layer** and listens on **`0.0.0.0:8080`**. Whoever could reach that port could expand one of 317 incidents and read a device password. Reach beyond the host depends on network/firewall (not verifiable from the repo).
- **Readers of the DB sinks:** `ops_dashboard_ro`, `incident_engine_rw` (SELECT), `postgres`.
- **Credentials implicated (rotation worklist, ids only):** hhm_credentials id=1 (GE/CT) used by 17 systems; hhm_credentials id=2 (GE/CT) used by 16 systems; hhm_credentials id=3 (GE/CT) used by 16 systems; hhm_credentials id=4 (GE/MRI) used by 2 systems; hhm_credentials id=5 (GE/MRI) used by 11 systems; hhm_credentials id=6 (GE/MRI) used by 5 systems; hhm_credentials id=7 (GE/CV/IR) used by 12 systems; hhm_credentials id=9 (Philips/CV/IR) used by 3 systems; hhm_credentials id=11 (Philips/CV/IR) used by 1 systems; hhm_credentials id=12 (Philips/CV/IR) used by 71 systems; hhm_credentials id=13 (Philips/CT) used by 22 systems; hhm_credentials id=15 (Philips/MRI) used by 24 systems; hhm_credentials id=19 (GE/CT) used by 3 systems; hhm_credentials id=23 (Philips/MRI) used by 1 systems
- **Recommendation:** (1) run [scripts/scrub-historical-credentials.sql](scripts/scrub-historical-credentials.sql) — safe-by-default (one transaction, ROLLBACK unless `-v COMMIT=1`, residual-zero verification), after the snapshot in its header; (2) `chmod o-r` the archive bundles now, then delete the 15 in-window bundles (their content is the same events the DB holds, scrubbed); (3) **rotate** the credential rows above — the exposure window was ~2 weeks across three sinks and a screen; (4) put the dashboard behind authentication or bind it to localhost (an ops decision outside this repo).
- **Severity:** HIGH (live exposure of equipment credentials; no code defect remains) · **Confidence:** High (counts measured directly; scrub pattern proven on synthetic text and sampled for JSON validity)
...
| SEC-011 | HIGH | Security | Pre-redactor plaintext credentials persist: 3 DB sinks (55,471 events), 15 world-readable archives (36,109), rendered by the unauthenticated dashboard (317 incidents) | util.app_run_logs; incidents.*; /opt/run-logs/.../archive; ops-dashboard | High | Run the safe-by-default scrub script; chmod/delete archives; rotate the listed credentials; auth the dashboard |
| SEC-002 | HIGH | Security | Decrypted passwords leak into cron out, app_run_logs, alert table via execFile error messages — **redactor half FIXED 2026-09-01; verified 100% effective over 6 days (27,750 masked, 0 unmasked). History: see SEC-011** | read/exec-hhm_data_grab.js:192,215,295,302 (+siblings) | High | Done: exec-error redactor. Remaining: env-passed secrets (SEC-004) |
