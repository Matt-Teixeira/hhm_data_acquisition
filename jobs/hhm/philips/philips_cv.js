...
async function get_philips_cv_data(run_id) {
  try {
    ...
    const systems = await getGeCtHhm(["Philips", "CV/IR"]);
    const credentials = await getHhmCreds(["Philips", "CV"]);
    console.log(systems);
    console.log(credentials);

    const cv_path = "./read/sh/philips/phil_cv_data_grab.sh";

    for await (const system of systems) {
      await log("info", "jobId", system.id, "get_philips_cv_data", "FN CALL", {
        exec_path: cv_path,
      });
    ...
    console.log(error);
    await log("error", run_id, "GE_CV", "get_philips_cv_data", "FN CALL", {
      error,
    });
  }
}
