...
**Confirmed live 2026-09-04:** the machine now reports honestly for the first time — and the report shows the connection and password both work; the machine simply has no log files where the script expects them. That's a question for the owner (wrong script for this machine, or a retired machine), no longer a hidden failure.
...
2. The VPN address book is going stale: the job that feeds it isn't on the schedule, only ever *adds* entries (never updates or removes them), and hides its own failures (BUG-006). Reactivated resets would be aiming at out-of-date targets. (Real-world example, 2026-09-08: a 50-minute network drop took 21 GE machines offline across a dozen sites; the app's map didn't know 20 of them. It recovered by itself, and the app reported it honestly throughout.)
